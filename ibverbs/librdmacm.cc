...
class rdma_cm_transport : public sstmac::sumi_transport
{
  RegisterAPI("rdma_cm", rdma_cm_transport)
  public:
    rdma_cm_transport(sprockit::sim_parameters* params,
...
rdma_cm_transport* sstmac_rdma_cm()
{
  sstmac::sw::thread* t = sstmac::sw::operating_system::current_thread();
  rdma_cm_transport* tp = t->get_api<rdma_cm_transport>();
  if (!tp->inited())
    tp->init();
  return tp;
}

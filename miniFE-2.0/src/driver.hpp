       ...
       Parameters& params, YAML_Doc& ydoc)
{
  ...
  compute_imbalance(global_box, my_box, largest_imbalance,
                                   std_dev, ydoc, true);
  ...
  Vector b(my_first_row, local_nrows);
  Vector x(my_first_row, local_nrows);
  ...
  if (matvec_with_comm_overlap) {
    time_kernels(A, b, x, matvec_overlap(), max_iters, rnorm, cg_times);
  }
  else {
    time_kernels(A, b, x, matvec_std(), max_iters, rnorm, cg_times);
  }
  num_iters = max_iters;
  ...
  if (matvec_with_comm_overlap) {
    ...
    matvec_overlap op;
    cg_solve(A, b, x, op, max_iters, tol,

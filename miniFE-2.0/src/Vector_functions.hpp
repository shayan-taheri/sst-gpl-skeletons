                  ...
                  const VectorType& vec)
{
  ...
  for(int p=0; p<numprocs; ++p) {
    if (p == myproc) {
      ...
      VectorType::GlobalOrdinalType first = vec.startIndex;
...
void sum_into_vector(size_t num_indices,
                     const VectorType::GlobalOrdinalType* indices,
                     const VectorType::ScalarType* coefs,
                     VectorType& vec)
{
  typedef VectorType::GlobalOrdinalType GlobalOrdinal;
...
//
void
  waxpby(VectorType::ScalarType alpha, const VectorType& x,
         VectorType::ScalarType beta, const VectorType& y,
         VectorType& w)
{
  typedef VectorType::ScalarType ScalarType;
...
// result - return-value
//
TypeTraits<Scalar>::magnitude_type
  dot(const Vector& x,
      const Vector& y)
{
  const MINIFE_LOCAL_ORDINAL n = x.coefs.size();

  typedef Vector::ScalarType Scalar;
  typedef TypeTraits<Vector::ScalarType>::magnitude_type magnitude;

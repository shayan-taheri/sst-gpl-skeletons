...
inline Scalar percentage_difference(Scalar value, Scalar average)
{
...
void get_global_min_max(NumericType local_n,
                        NumericType& global_n,
                        NumericType& min_n,
                        int& min_proc,
                        NumericType& max_n,
                        int& max_proc)
...
//
  ...
  std::vector<NumericType> all_n(numprocs, 0);
  all_n[myproc] = local_n;
#ifdef HAVE_MPI
  std::vector<NumericType> tmp(all_n);
  MPI_Datatype mpi_dtype = TypeTraits<NumericType>::mpi_type();
...
inline Scalar compute_std_dev_as_percentage(Scalar local_nrows,
                                     Scalar avg_nrows)
{
...
inline GlobalOrdinal find_row_for_id(GlobalOrdinal id,
                              const std::map<GlobalOrdinal,GlobalOrdinal>& ids_to_rows)
{
  std::map<GlobalOrdinal,GlobalOrdinal>::const_iterator

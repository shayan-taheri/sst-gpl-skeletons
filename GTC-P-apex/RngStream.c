...
static int rng_stream_global_offset = 0;

static int rng_stream_is_initialized = 0;
        ...
        int nstreams, double *nextSeed_local)
{
    ...
    int offset, i;
    //double nextSeed_local = { 12345, 12345, 12345, 12345, 12345, 12345 };

    //if (rng_stream_global_offset == 0) { 
   
       offset = (proc_num * nthreads * nstreams) + 
            (thread_num * nstreams);

       //fprintf(stderr, "thread %d, offset %d\n", thread_num, offset);

       for (i = 0; i<offset; i++) {
           MatVecModM (A1p127, nextSeed_local, nextSeed_local, m1);
           MatVecModM (A2p127, &nextSeed_local[3], &nextSeed_local[3], m2);
       }

       // rng_stream_global_offset = nprocs * nthreads * nstreams;
   
       // rng_stream_is_initialized = 1;
/*     
     } else {

        if ((proc_num == 0) && (thread_num == 0))
            fprintf(stderr, "Warning: RngStream has already been initialized once. "
                "Continuing with next available stream\n");

       offset = rng_stream_global_offset + (proc_num * nthreads * nstreams) + 
            (thread_num * nstreams);
...
RngStream RngStream_CreateStream (const char name[], double *nextSeed_local)
{
   int i;
   RngStream g;
   //size_t len;
   ...
   /*
   if (rng_stream_is_initialized == 0) {
        fprintf(stderr, "Error! Call RngStream_ParInit first.\n");
   }
   */
   ...
   */
   ...
   for (i = 0; i < 6; ++i) {
      g->Bg[i] = g->Cg[i] = g->Ig[i] = nextSeed_local[i];
   }
   MatVecModM (A1p127, nextSeed_local, nextSeed_local, m1);
   MatVecModM (A2p127, &nextSeed_local[3], &nextSeed_local[3], m2);
   return g;
...
void RngStream_FastAdvanceState (RngStream g, long e, long c)
{

  long k, klog2;

  RngStream_AdvanceState( g, e, 0 );

  k = 1;
  klog2=0;
  if( k & c )
    RngStream_AdvanceState( g, 0, 1 );
  
  for( k=2, klog2=1; k<=c; k*=2, klog2++  ){
    if( k & c )
      RngStream_AdvanceState( g, klog2, 0 );
  }
...
{
   return i + (unsigned int) ((j - i + 1.0) * RngStream_RandU01 (g));
}

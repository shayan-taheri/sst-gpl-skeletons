...
int poisson(int iflag, gtc_bench_data_t *gtc_input) {
    ...
    int nloc_over, ipsi_in, ipsi_out, igrid_in, igrid_nover_in_radiald, igrid_nover_out_radiald,
      igrid_nover_start, igrid_nover_end, ipsi_nover_in_radiald, ipsi_nover_out_radiald;
    ...
    igrid_nover_in_radiald = radial_decomp->igrid_nover_in_radiald;
    igrid_nover_out_radiald = radial_decomp->igrid_nover_out_radiald;
    igrid_nover_start = igrid_nover_in_radiald - igrid_in;
    igrid_nover_end = igrid_nover_out_radiald - igrid_in;
...
#pragma omp parallel for
	  for (int i=igrid_nover_start; i<igrid_nover_end+1; i++) {
	    ...
	    for (int i=ipsi_nover_in_radiald; i<ipsi_nover_out_radiald+1; i++){
	      phitmp[igrid[i]-igrid_in] = phitmp[igrid[i]+mtheta[i]-igrid_in];
	    }
...
#endif
            if (radial_decomp->nproc_radiald>1)
              fix_radial_ghosts(gtc_input, phitmp, 0, 1);
#if FINE_TIMER
            double end_t=timer();
            grid_t_comm2 += (end_t - start_t);
#endif
#endif

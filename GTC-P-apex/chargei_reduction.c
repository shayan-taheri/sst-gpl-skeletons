...
#pragma omp for
    for (m = 0; m < mi; m++) {
	  ...
	  if (abs(rhotmp)>rho_max) {
	    printf("warning: reducing rhoi to %e from %e\n", (rhotmp/abs(rhotmp))*rho_max/pgyro_lc[larmor], rhotmp);
	    rhotmp = (rhotmp/abs(rhotmp))*rho_max;
	  ...
	  if (abs(rhotmp) > rho_max) {
            printf("warning: reducing rhoi to %e from %e\n", (rhotmp/abs(rhotmp))*rho_max/pgyro[idx1], rhotmp);
            rhotmp = (rhotmp/abs(rhotmp))*rho_max;
...
#pragma sst replace nthreads sstmac_omp_get_num_threads()
...
#pragma omp barrier 
    ...
    if (tid==0){
      end_t1 = timer();
      charge_t_comp_t1 += (end_t1 - start_t1);
    }
#endif
...
int chargei_init(gtc_bench_data_t* gtc_input) {
    ...
    /* temporary variables */
    real psitmp, thetatmp, zetatmp, rhoi, r, wzt;
    ...
    for (m = 0; m < mi; m++) {
        ...
        //BMA assert(ip>=ipsi_valid_in);
        //BMA assert(ip<=ipsi_valid_out);
        ...
        for (larmor = 0; larmor < 4; larmor++) {
            ...
            //BMA assert(ii>=ipsi_in);
            //BMA assert(ii<=ipsi_out-1);
